...
const apiUrl = args[0] || 'https://oauth-proxy-ias.c-127c9ef.stage.kyma.ondemand.com';
...
class TokenService {
  ...
  async getToken(serviceName) {
    try {
      ...
      console.log(`🔐 Token acquired for ${serviceName}: ${token.access_token.substring(0, 50)}...`);
      //print the token claims
      console.log(`🔐 Token claims: ${JSON.stringify(jwtDecode(token.access_token), null, 2)}`);
      return token;       
